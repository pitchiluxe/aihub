...
import {
  ...
  Wand2, Key, Newspaper, FlaskConical,
} from "lucide-react";
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-white">AI Tube</h2>
        ...
        <div className="flex items-center gap-2 sm:ml-auto flex-wrap">
          ...
          >
            {loading ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Fetching…</> : <><RotateCcw className="w-3.5 h-3.5" />Refresh</>}
          </button>
...
export default function MillionIdeasPage() {
  const [pageTab, setPageTab] = useState<"ideas" | "tube">("ideas");
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#070b12]">
      ...
      {/* ── Page Tab Switcher ─────────────────────────────────── */}
      <div className="flex gap-1 px-4 md:px-8 pt-4 border-b border-white/5">
        {([
          { id: "ideas", label: "💡 Ideas", desc: "Business ideas + starter kits" },
          { id: "tube",  label: "▶ Tube",   desc: "Top AI YouTube videos" },
        ] as const).map((tab) => (
          <button
            key={tab.id}
